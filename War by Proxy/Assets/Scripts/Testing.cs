...
public class Testing : MonoBehaviourPun
{
    ...
    //public GameGUI localGameGui = canvas.GetComponent<GameGUI>();
    public GameObject selectedTile;
    public GameObject list;
    public GameObject listContent;
    public GameObject mapButtonPrefab;
    /*public int[,] damageMatrix = new int[6,6] { {55, 45, 12, 5, 15, 7}, 
                                                {65, 55, 85, 55, 70, 9},
                                                {70, 65, 35, 6, 45, 12},
                                                {75, 70, 85, 55, 70, 10},
                                                {90, 85, 80, 70, 75, -1},
    ...
    public void PrintListUnits(List<Unit> array, int x, int z, int team)
    {
        ...
        {
            GameObject mapButton = Instantiate(mapButtonPrefab, listContent.transform);
            mapButton.transform.Find("Text").GetComponent<Text>().text = item.ToString();
            Debug.Log("item: " + item + ", x: " + x + ", z: " + z + ", team: " + team);
            mapButton.GetComponent<Button>().onClick.AddListener(() => photonView.RPC("RecruitUnit", RpcTarget.All, item, x, z, team));
        }*/
    }
    ...
    public void PrintListCityUpgrades(List<City> array, int x, int z, Transform tile, int team)
    {
        ...
        foreach(TilemapObject.TilemapSprite item in array)
        {
            GameObject mapButton = Instantiate(mapButtonPrefab, listContent.transform);
            mapButton.transform.Find("Text").GetComponent<Text>().text = item.ToString();
            mapButton.GetComponent<Button>().onClick.AddListener(() => photonView.RPC("UpgradeCity", RpcTarget.All, item, x, z, tile.position, team));
            mapButton.GetComponent<Button>().onClick.AddListener(() => photonView.RPC("RPCDestroy", RpcTarget.All, tile.gameObject.name));
        }*/
    }
    ...
    public void RecruitUnit(Unit.UnitType type, int x, int z, int team)
    {
        ...
        /*GameObject unitInstance = Instantiate(un.unitVisualPrefab, new Vector3(x * 2, 0.7f, z * 2), Quaternion.identity, map);
        unitInstance.name = un.ToString() + x + z;
        unitmap.GetGrid().GetGridObject(x, z).SetUnitVisual(unitInstance);
        un.Visualize(teamColours[team], unitInstance);*/
    ...
    {
        //Transform tile = new GameObject().transform;
        //tile.position = pos;
    ...
    private void Start()
    {
        ...
        if(debugMode == 1)
        {
            grid = new GameGrid<int>(mapSizeX, mapSizeZ, 2f, Vector3.zero, true, (GameGrid<int> g, int x, int z) => 0);
            unitmap = new Unitmap(mapSizeX, mapSizeZ, 2f, Vector3.zero);
            unitmap.InsertUnit(2, 4, Unit.UnitType.Infantry, 0, 0, 99, null, 0);
            ...
            for (int z = 0; z < mapSizeZ; z++)
            {
                ...
                {
                    if (tilemap.GetIntFromSprite(x, z) == 3)
                    {
        ...
        if (debugMode == 1 || debugMode == 2)
        {
            ...
            for (int z = 0; z < mapSizeZ; z++)
            {
                for (int x = 0; x < mapSizeX; x++)
                {
                    ...
                    Instantiate(tt.tileVisualPrefab, new Vector3(x * 2, 0, z * 2), Quaternion.identity, map);
                    if(unitmap.GetGrid().GetGridObject(x, z) != null)
                    {
                        UnitInstantiate(unitTypes[unitmap.GetGrid().GetGridObject(x, z).GetIntFromUnit()], x, z, 0);
                        /*GameObject unitInstance = Instantiate(unitTypes[unitmap.GetGrid().GetGridObject(x, z).GetIntFromUnit()].unitVisualPrefab, new Vector3(x * 2, 0.7f, z * 2), Quaternion.identity, map);
                        unitInstance.name = unitTypes[0].ToString() + x + z;
                        unitmap.GetGrid().GetGridObject(x, z).SetUnitVisual(unitInstance);
                        unitTypes[unitmap.GetGrid().GetGridObject(x, z).GetIntFromUnit()].Visualize(teamColours[0], unitInstance);*/
                    }
                }
            }
        } 
        else if (debugMode == 3 || debugMode == 4)
        {
            tilemap.Load("Main_Tiles");
            unitmap.Load("Main_Units");
            for (int z = 0; z < mapSizeZ; z++)
            {
                for (int x = 0; x < mapSizeX; x++)
                {
                    ...
                    if(unitmap.GetGrid().GetGridObject(x, z) != null)
                    {
                        ...
                        /*GameObject unitInstance = Instantiate(unitTypes[un.GetIntFromUnit()].unitVisualPrefab, new Vector3(x * 2, 0.7f, z * 2), Quaternion.identity, map);
                        unitInstance.name = un.ToString() + x + z;
                        unitmap.GetGrid().GetGridObject(x, z).SetUnitVisual(unitInstance);
                        unitTypes[un.GetIntFromUnit()].Visualize(teamColours[un.GetTeam()], unitInstance);*/
                    }
                }
            }
            Debug.Log(tilemap.GetGrid().GetGridObject(0, 0).GetType());
        }
    }
    ...
    {
        Debug.Log("Parameters for UnitArrayShiftLeft method: " + array.Length + ", " + startindex);
        ...
        //Unit[] newarray = new Unit[array.Length - startindex];
    ...
    public void RemovePlayerFromGame(int PlayerID)
    {
        ...
        {
            Debug.Log("Attempting to destroy unit: " + destroyUnit.ToString() + destroyUnit.GetX() + destroyUnit.GetZ());
        ...
        {
            Debug.Log("Attempting to destroy building: " + destroyBuilding.ToString() + destroyBuilding.GetX() + destroyBuilding.GetZ());
            destroyBuilding.SetTeam(0);
            RPCDestroy(destroyBuilding.ToString() + destroyBuilding.GetX() + destroyBuilding.GetZ());
            if(destroyBuilding.GetTilemapSprite() == TilemapObject.TilemapSprite.HQ || destroyBuilding.GetType().IsSubclassOf(typeof(Building)))
            {
    ...
    {
        Debug.Log(startx + ", " + startz + ", " + endx + ", " + endz);
    ...
    {
        Debug.Log(attackerx + ", " + attackerz + ", " + defenderx + ", " + defenderz);
    ...
    private void Update()
    {
        if(debugMode == 3)
        {
            ...
            /*if (Input.GetKeyDown(KeyCode.Alpha1)) { builder = 0; currentPlacer = 0; Debug.Log("Road"); }
            if (Input.GetKeyDown(KeyCode.Alpha2)) { builder = 1; currentPlacer = 0; Debug.Log("Plains"); }
            if (Input.GetKeyDown(KeyCode.Alpha3)) { builder = 2; currentPlacer = 0; Debug.Log("Forest"); }
            if (Input.GetKeyDown(KeyCode.Alpha4)) { builder = 3; currentPlacer = 0; Debug.Log("Mountains"); }
            if (Input.GetKeyDown(KeyCode.Alpha5)) { builder = 4; currentPlacer = 0; Debug.Log("Sea"); }
            if (Input.GetKeyDown(KeyCode.Alpha6)) { builder = 5; currentPlacer = 0; Debug.Log("Shore"); }
            if (Input.GetKeyDown(KeyCode.Alpha7)) { builder = 6; currentPlacer = 2; Debug.Log("City0"); }
            if (Input.GetKeyDown(KeyCode.Alpha8)) { builder = 7; currentPlacer = 2; Debug.Log("City1"); }
            if (Input.GetKeyDown(KeyCode.Alpha9)) { builder = 8; currentPlacer = 2; Debug.Log("City2"); }
            if (Input.GetKeyDown(KeyCode.Q)) { unitSetter = 0; currentPlacer = 1; Debug.Log("Neutral Infantry"); }
            if (Input.GetKeyDown(KeyCode.W)) { unitSetter = 1; currentPlacer = 1; Debug.Log("Red Infantry"); }
            if (Input.GetKeyDown(KeyCode.E)) { unitSetter = 2; currentPlacer = 1; Debug.Log("Blue Infantry"); }*/

            if (Input.GetMouseButton(1))
            {
                if (builder == -1) { builder = 0; Debug.Log("Plains"); }
                if (unitSetter == -1) { unitSetter = 0;}
                //Debug.Log(builder + "," + currentPlacer);
                if (MouseClickDetector(ref x, ref z, ref tile))
                {
                    if (0 <= builder && builder <= 7)
                    {
                        //Debug.Log("Placin a terrain");
                    ...
                    else if (16 <= builder && builder <= 35)
                    {
                        if(unitmap.GetGrid().GetGridObject(tile.position) == null)
                        {
                            ...
                            //unitmap.GetGrid().SetGridObject(tile.position, unitTypes[0]);
                            UnitInstantiate(un, x, z, currentPlacer);
                            /*GameObject unitInstance = Instantiate(un.unitVisualPrefab, new Vector3(x * 2, 0.7f, z * 2), Quaternion.identity, map);
                            unitInstance.name = un.ToString() + x + z;
                            unitmap.GetGrid().GetGridObject(x, z).SetUnitVisual(unitInstance);
                            un.Visualize(teamColours[currentPlacer], unitInstance);*/
                        }
                        else
                        {
            ...
            {
                tilemap.Save("Main_Tiles");
                unitmap.Save("Main_Units");
                Debug.Log("Saved!");
            }
        ...
        } else if(debugMode == 2)
        {
            /*if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    Transform tile = hit.transform;
                    pathfinding.GetGrid().GetXZ(tile.position, out int x, out int z);
                    List<PathNode> path = pathfinding.FindPath(0, 0, x, z);
                    if (path != null)
                    {
                        for (int i = 0; i < path.Count - 1; i++)
                        {
                            Debug.DrawLine(new Vector3(path[i].x, 0.25f, path[i].z) * 2f, new Vector3(path[i + 1].x, 0.25f, path[i + 1].z) * 2f, Color.green, 100f);
                        }
                    }
                }
            }*/
        } else if(debugMode == 1)
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                {
                    grid.SetGridObject(lastSelected.position, 0);
                    lastSelected = tile;
                }
                grid.SetGridObject(tile.position, 1);
            }

            if (Physics.Raycast(ray, out hit) && Input.GetMouseButtonDown(1))
            {
                Debug.Log(grid.GetGridObject(hit.transform.position));
            }
        } else if(debugMode == 4)
        {
            if (localPlayerID == turnCounter)
            {
                            ...
                            if((graph[0].x == x && graph[0].z == z) || unitmap.GetGrid().GetGridObject(x, z) == null)
                            {
                                ...
                                if(unitmap.GetGrid().GetGridObject(x, z).GetLoadCapacity() != 0 /*&& unitmap.GetGrid().GetGridObject(x, z).GetLoadedUnits()[0] != null*/)
                                {
                                    ...
                                    for(int i = 0; i < unitmap.GetGrid().GetGridObject(x, z).GetLoadCapacity(); i++)
                                    {
                                        ...
                                        {
                                            Debug.Log(unitmap.GetGrid().GetGridObject(x, z).GetLoadedUnits()[i].ToString());
                ...
                else if (Input.GetMouseButtonDown(1) && unitSelected == "fire")
                {
                    if (MouseClickDetector(ref x, ref z, ref tile))
                    {
                        ...
                        //Debug.Log(targetables[0].GetX());
                        //Debug.Log(targetables[0].GetZ());
                        //Debug.Log(unitmap.GetGrid().GetGridObject(targetables[0].GetX(), targetables[0].GetZ()));
                        ...
                        {
                            //Debug.Log("Checking for selected position: " + graph.Count);
                            //unitmap.MoveUnit(graph[0].x, graph[0].z, x, z);
                ...
                /*else if (Input.GetMouseButtonDown(0) && menuUp == false && MouseClickDetector(ref x, ref z, ref tile) && unitmap.GetGrid().GetGridObject(x, z) != null && unitmap.GetGrid().GetGridObject(x, z).GetTeam() != playersInMatch[localPlayerID].GetTeam())
                {
                    pathmaking.ClearGrid();
                    targetables.Clear();
                    graph = pathmaking.CreateReachableGraph(x, z, unitmap.GetGrid().GetGridObject(x, z), tilemap, false);
                    if (graph != null)
                    {
                    ...
                    if (MouseClickDetector(ref x, ref z))
                    {
                        if(localTurnSystem.GetUnitsAwaitingOrders().Contains(unitmap.GetGrid().GetGridObject(x, z)))
                        {
                            ...
                            if (graph != null)
                            {
                                for (int i = 0; i < graph.Count; i++)
                                {
                                    /*foreach(DijkstraNode node in graph[i].connectedFromNodes)
                                    {
                                        Debug.DrawLine(new Vector3(graph[i].x, 0.25f, graph[i].z) * 2f, new Vector3(node.x, 0.25f, node.z) * 2f, Color.green, 100f);
                                    }*/
                                    //Debug.DrawLine(new Vector3(graph[i].x, 0.25f, graph[i].z) * 2f, new Vector3(graph[i + 1].x, 0.25f, graph[i + 1].z) * 2f, Color.green, 100f);
                                    //Debug.Log(graph[i].nodePosition() + "'s " + graph[i].connectedFromNodesElements());
                                    selectedTiles.Add(Instantiate(selectedTile, new Vector3(graph[i].x * 2, 0.1f, graph[i].z * 2), Quaternion.identity, map));
                                }
                                unitSelected = "move";
                                /*foreach(GameObject selected in selectedTiles)
                ...
                else if (Input.GetMouseButtonDown(1) && menuUp == false)
                {
                    ...
                    if (MouseClickDetector(ref x, ref z))
                    {
                        if(localTurnSystem.GetUnitsAwaitingOrders().Contains(unitmap.GetGrid().GetGridObject(x, z)))
                        {
                            ...
                            //Debug.Log(targetables[0].GetX());
                            //Debug.Log(targetables[0].GetZ());
                            Debug.Log(targetables[0]);
                            if (targetables != null && targetables.Count > 1)
                            {
                                for (int i = 1; i < targetables.Count; i++)
                                {
                                    selectedTiles.Add(Instantiate(selectedTile, new Vector3(targetables[i].GetX() * 2, 0.1f, targetables[i].GetZ() * 2), Quaternion.identity, map));
                ...
                else if (Input.GetKeyDown(KeyCode.X))
                {
                    if(MouseClickDetector(ref x, ref z))
                    {
                        Debug.Log("Current X/Z values are: " + x + "/" + z);
                    }
                }
                else if (Input.GetKeyDown(KeyCode.M))
                {
                    int unitscounter = 1;
                    foreach(Unit playerunit in playersInMatch[localPlayerID].GetUnloadedUnits())
                    {
                        Debug.Log("Unit #" + unitscounter + ": " + playerunit.ToString() + " at " + playerunit.GetX() + "," + playerunit.GetZ());
                        unitscounter++;
                    }
                    foreach(Unit playerunit in playersInMatch[localPlayerID].GetLoadedUnits())
                    {
                        Debug.Log("Unit #" + unitscounter + ": " + playerunit.ToString() + " loaded in ");
                        unitscounter++;
                    }
                }
            }
